...
class CordexContinuous(BaseOptimizer):
    ...
    def optimize(self, runs, nx, scalars=0, epochs=1000, final_pass_iter=100):
        ...
        for _ in tqdm(range(epochs)):
            ...
            model_matrix = Gamma_
            best_design, best_optimality_value = self._cordex_loop(model_matrix, runs, nx, scalars,
                                                                   best_optimality_value, best_design)

        if final_pass_iter > 0:
            for _ in tqdm(range(final_pass_iter)):
                best_design, best_optimality_value = self._cordex_loop(best_design, runs, nx, scalars,
                                                                       best_optimality_value, best_design)

        return best_design, np.abs(best_optimality_value)

    def _cordex_loop(self, model_matrix, runs, nx, scalars, best_optimality_value, best_design):
